...
permission:
  ...
  bash:
    ...
    "just agent::dispatch-start *": allow
    "just agent::dispatch-status *": allow
    "just agent::dispatch-respond *": allow
    "just agent::dispatch-stop *": allow
    "just automation::maintenance-check *": allow
    "just automation::dispatch-start *": allow
    "just automation::dispatch-status *": allow
    "just automation::dispatch-respond *": allow
    "just automation::dispatch-stop *": allow
...
For an existing already-launched resumable Task, do not call any task-start or hydration path. Run only the dedicated `just agent::contract-resume-check <task>` from Main, then use `just agent::dispatch-start <task>` only when its result is exactly `status: READY` with `mode: resume`. The dispatcher revalidates and passes the complete READY evidence, including `task`, `worktree`, `taskStatus`, and `sha256`, to exactly one `task-orchestrator`, fixed internally. A missing, non-canonical, non-READY, wrong-mode, or incomplete result blocks launch. `integration-pending`, `merged`, and `cancelled` are not resumable; post-publication reconciliation remains Main-owned.

Automation Maintenance is a separate receipt-driven lifecycle. When the user explicitly selects an existing registered Automation Maintenance Task, run `just automation::maintenance-check <task>` instead of either normal contract readiness command, then use `just automation::dispatch-start <task>` only when the result is `status: READY`, `mode: maintenance`. The dispatcher revalidates the complete evidence, derives the trusted source path and immutable revision from an active or consumed maintenance receipt, and binds exactly one fixed `maintenance-orchestrator`; callers cannot provide a path or agent. A pristine maintenance Task without that receipt remains blocked. Maintenance readiness may safely resume an `initialized` Task from applied, committed, pushed, or Draft-PR evidence without making `initialized` generally resumable and without fabricating normal product-Task states. Never route such a Task through `/task-run`.

Use the matching `dispatch-status` command to reconcile the child session. If it reports an exact pending permission ID and pattern, ask the human without broadening or summarizing the pattern, then relay only the selected `once`, `session`, or `deny` decision with `dispatch-respond`. Use `dispatch-stop` only for the selected Task; it never cleans up a worktree. Never use `--auto`, an arbitrary host, port, password, session ID, agent, or target path.
