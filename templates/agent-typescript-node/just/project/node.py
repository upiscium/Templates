...
NUMBER = r"(?:0|[1-9]\d{0,5})"
PARTIAL_VERSION = re.compile(
    rf"^({NUMBER})(?:\.({NUMBER}|x|X|\*))?(?:\.({NUMBER}|x|X|\*))?$"
)
RUNTIME_VERSION = re.compile(
    rf"^v?({NUMBER})\.({NUMBER})\.({NUMBER})(?:\+[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?$"
)
NPM_VERSION = re.compile(rf"^({NUMBER})(?:\.({NUMBER})(?:\.({NUMBER}))?)?$")
MAX_ENGINE_LENGTH = 256
MAX_ENGINE_CONSTRAINTS = 8
...
def version(text: str) -> tuple[int, int, int]:
    match = RUNTIME_VERSION.fullmatch(text.strip())
    if not match:
...
def partial(text: str) -> tuple[int, int | None, int | None]:
    match = PARTIAL_VERSION.fullmatch(text.strip())
    if not match:
        raise NodeError(f"unsupported Node engine syntax: {text!r}")
    values = match.groups()
    minor = None if values[1] in (None, "x", "X", "*") else int(values[1])
    patch = None if values[2] in (None, "x", "X", "*") else int(values[2])
    if minor is None and values[2] is not None:
        raise NodeError(f"unsupported Node engine syntax: {text!r}")
    return int(values[0]), minor, patch


def satisfies_node(spec: str, actual: tuple[int, int, int]) -> bool:
    spec = spec.strip()
    if not spec or len(spec) > MAX_ENGINE_LENGTH or "||" in spec:
        raise NodeError("unsupported Node engine syntax")
    parts = spec.split()
    if len(parts) > MAX_ENGINE_CONSTRAINTS:
        raise NodeError("unsupported Node engine syntax")
    constraints: list[tuple[str, tuple[int, int, int]]] = []
    for item in parts:
        match = re.fullmatch(
            rf"(>=|<=|>|<|\^|~|=)?(v?{NUMBER}(?:\.(?:{NUMBER}|x|X|\*)(?:\.(?:{NUMBER}|x|X|\*))?)?)",
            item,
        )
        if not match:
...
def metadata(root: Path) -> dict:
    ...
    if manager is not None and (
        ...
        or not manager.startswith("npm@")
        or NPM_VERSION.fullmatch(manager.removeprefix("npm@")) is None
    ):
    ...
    for field in ("name", "version"):
        if not isinstance(package.get(field), str) or not package[field]:
            raise NodeError(f"package.json {field} must be a non-empty string")
        if not isinstance(lock.get(field), str) or not isinstance(lock_root.get(field), str):
            raise NodeError(f"package-lock.json root {field} must be a string")
    ...
    for field in ("dependencies", "devDependencies", "optionalDependencies"):
        ...
        if not isinstance(declared, dict) or any(
            not isinstance(dependency, str) or not isinstance(requirement, str)
            for dependency, requirement in declared.items()
        ):
...
def validate_runtime(package: dict) -> None:
    ...
    if manager is None:
        return
    declared_match = NPM_VERSION.fullmatch(manager.removeprefix("npm@"))
    if declared_match is None:  # metadata() validates this; keep direct callers fail-closed.
        raise NodeError("packageManager must be a compatible npm version")
    declared = tuple(int(part) for part in declared_match.groups() if part is not None)
    npm_text = subprocess.check_output(["npm", "--version"], text=True).strip()
    npm_match = NPM_VERSION.fullmatch(npm_text)
    if npm_match is None or any(part is None for part in npm_match.groups()):
        raise NodeError(f"malformed npm version: {npm_text!r}")
    actual_npm = tuple(int(part) for part in npm_match.groups() if part is not None)
