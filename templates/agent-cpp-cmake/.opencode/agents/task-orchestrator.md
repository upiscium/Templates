...
permission:
  ...
  bash:
    ...
    "just agent::local-delete *": ask
    "just agent::local-delete *$*": deny
    "just agent::local-delete *`*": deny
    "just agent::local-delete *;*": deny
    "just agent::local-delete *&*": deny
    "just agent::local-delete *|*": deny
...
- For a local-filesystem-delete Ask, use only `just agent::local-delete <relative-target> [recursive]`; its guarded API proves the target stays inside the current Task worktree without a root, protected path, absolute paths, external paths, parent traversal, or symlink-resolved escape. Never use raw `rm`/`rmdir` or this Ask path for repository history, remote, privilege, system-store, or external-directory destruction.
