...
class TypeScriptNodeAdapterTest(unittest.TestCase):
    ...
    def test_node_engine_contract(self) -> None:
        ...
        for malformed in ("22.x.1", "01.2.3", "22.000001.0", "22.1.2-beta.1"):
            with self.subTest(malformed=malformed), self.assertRaises(node_adapter.NodeError):
                node_adapter.satisfies_node(malformed, (22, 1, 2))
        for malformed in ("v01.2.3", "v1.2.3+bad..metadata", "v1000000.0.0"):
            with self.subTest(malformed=malformed), self.assertRaises(node_adapter.NodeError):
                node_adapter.version(malformed)
        for oversized in ("22 " * 9, "2" * 257):
            with self.subTest(oversized=oversized[:20]), self.assertRaises(node_adapter.NodeError):
                node_adapter.satisfies_node(oversized, (22, 1, 2))
    ...
    def test_package_manager_and_runtime_versions_are_strict(self) -> None:
        temporary, root = self.make_project()
        self.addCleanup(temporary.cleanup)
        package = json.loads((root / "package.json").read_text(encoding="utf-8"))
        for malformed in ("npm@010", "npm@10.01", "npm@1000000", "pnpm@10"):
            with self.subTest(malformed=malformed):
                package["packageManager"] = malformed
                (root / "package.json").write_text(json.dumps(package), encoding="utf-8")
                with self.assertRaisesRegex(node_adapter.NodeError, "compatible npm version"):
                    node_adapter.metadata(root)
        package["packageManager"] = "npm@10"
        for malformed in ("10", "10.9", "010.9.0", "10.9.0-beta.1"):
            with self.subTest(malformed=malformed), patch.object(
                node_adapter.subprocess,
                "check_output",
                side_effect=lambda command, value=malformed, **_: (
                    "v22.11.0\n" if command[0] == "node" else value + "\n"
                ),
            ), self.assertRaisesRegex(node_adapter.NodeError, "malformed npm version"):
                node_adapter.validate_runtime(package)
    ...
    def test_omitted_scripts_are_skipped(self) -> None:
        temporary, root = self.make_project()
        self.addCleanup(temporary.cleanup)
        package = json.loads((root / "package.json").read_text(encoding="utf-8"))
        package.pop("scripts")
        with patch.object(node_adapter.subprocess, "run") as run:
            self.assertEqual(node_adapter.run(root, package, "lint"), 0)
            run.assert_not_called()
    ...
    def test_missing_lockfile_root_identity_fails_closed(self) -> None:
        temporary, root = self.make_project()
        self.addCleanup(temporary.cleanup)
        for location in ("package", "lock", "lock-root"):
            with self.subTest(location=location):
                ...
                if location == "package":
                    ...
                    lock["packages"][""].pop("name")
                (root / "package.json").write_text(json.dumps(package), encoding="utf-8")
                (root / "package-lock.json").write_text(json.dumps(lock), encoding="utf-8")
                with patch.object(node_adapter.shutil, "which", return_value="/tool"), self.assertRaisesRegex(
                    node_adapter.NodeError, "name"
                ):
                    node_adapter.metadata(root)
                temporary.cleanup()
                temporary, root = self.make_project()
    ...
    def test_scoped_root_dependency_resolves_from_lockfile(self) -> None:
        temporary, root = self.make_project()
        self.addCleanup(temporary.cleanup)
        package = json.loads((root / "package.json").read_text(encoding="utf-8"))
        lock = json.loads((root / "package-lock.json").read_text(encoding="utf-8"))
        package["name"] = "@scope/application"
        package["dependencies"] = {"@scope/library": "1.0.0"}
        lock["name"] = package["name"]
        lock["packages"][""]["name"] = package["name"]
        lock["packages"][""]["dependencies"] = package["dependencies"]
        lock["packages"]["node_modules/@scope/library"] = {"version": "1.0.0"}
        (root / "package.json").write_text(json.dumps(package), encoding="utf-8")
        (root / "package-lock.json").write_text(json.dumps(lock), encoding="utf-8")
        with patch.object(node_adapter.shutil, "which", return_value="/tool"):
            self.assertEqual(node_adapter.metadata(root)["name"], "@scope/application")
